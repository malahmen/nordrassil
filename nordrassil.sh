...
_detect_running_target() {
    ...
    if [[ "$chosen" == "k8s" ]]; then
        ...
        # '|| K8S_POD=""': a failing kubectl must fall through to the warn
        # below, not kill the script silently under set -e (stderr is muted).
        K8S_POD=$(kubectl $K8S_CTX_FLAGS get pods -n "$K8S_NAMESPACE" -l app=vanilla-wow-server -o jsonpath='{.items[0].metadata.name}' 2>/dev/null) || K8S_POD=""
        if [[ -z "$K8S_POD" ]]; then
